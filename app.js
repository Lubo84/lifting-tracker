...
const PROGRAM = {
  ...
  // Exercise catalog for each day with categories
  days: [
...
function loadState(){
  ...
  try{
    ...
    // backfill reminders defaults if missing
    if(!s.reminders){
...
function renderWorkout(){
  ...
  day.exercises.forEach((ex, idx)=>{
    ...
    // determine default sets count number
    let setsCount = 3;
    ...
    if(!entry.exercises[exKey]){
      ...
      // if rules changed set count (phase change), adjust length politely
...
document.addEventListener("DOMContentLoaded", ()=>{
  ...
  startTimerBtn.addEventListener("click", ()=>{
    // Default rest based on selected day's first exercise category
    const rules = getRules("main");
    // pick seconds: parse first number in rest string
    let seconds = 90;
    if(rules && rules.rest){
...
const preset120 = document.getElementById("preset120");
if(preset60){ preset60.addEventListener("click", ()=>startTimer(60)); }
if(preset90){ preset90.addEventListener("click", ()=>startTimer(90)); }
if(preset120){ preset120.addEventListener("click", ()=>startTimer(120)); }
